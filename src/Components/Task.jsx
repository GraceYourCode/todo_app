...
const Task = ({ dark_Mode, task, delete_Task }) => {
  ...
  return (
    <div className={`flex ${dark_Mode ? "bg-desaturated_Blue" : "bg-light_Grey"} px-3 md:px-4 lg:px-6 py-4 gap-3.5 lg:gap-5 justify-between items-center mb-0.5`}
      ...
      <p className={`w-full text-xs md:text-sm ${dark_Mode ? (task.completed ? "text-dark_Grey_Blue" : "text-light_Grey_Blue") :
        (task.completed ? "text-light_Mode_Dark_Grey_Blue" : "text-light_Mode_Very_Dark_Grey_Blue")} 
      ${task.completed && "line-through"}`}>{task.task}</p>
      {type == "landscape-primary" && <button className="hidden" ref={del_Btn} onClick={() => delete_Task(task.id)}>
      ...
      <button className="portrait:block hidden" onClick={() => delete_Task(task.id)}>
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"><path fill="#494C6B" fillRule="evenodd" d="M16.97 0l.708.707L9.546 8.84l8.132 8.132-.707.707-8.132-8.132-8.132 8.132L0 16.97l8.132-8.132L0 .707.707 0 8.84 8.132 16.971 0z" /></svg>
      </button>
    </div>
  )
